...
const getSimilarity = (s1: string, s2: string): number => {
  ...
  return 1.0 - matrix[len2][len1] / Math.max(len1, len2);
};
...
const VoiceOrder = ({
  ...
  const findMatch = useCallback(
    (text: string) => {
      ...
      // Clean query of punctuation for robust scanning
      ...
      // 1. DETECT ACTIONS
      const shouldSave = ["save", "hold", "rakh lo", "rakho", "karo save"].some(
        (k) => cleanQuery.includes(k),
      );
      const shouldKOT = ["kot", "kitchen", "parcha", "parchi", "ticket"].some(
        (k) => cleanQuery.includes(k),
      );
      const shouldBill =
        ["bill", "bhugtan", "payment", "final", "print bill"].some((k) =>
          cleanQuery.includes(k),
        ) && !shouldKOT;
      const shouldGenericPrint =
        ["print", "nikalo", "generate"].some((k) => cleanQuery.includes(k)) &&
        !shouldBill &&
        !shouldKOT;
      ...
      let isSelectAll =
        allTriggers.some((t) => cleanQuery.includes(t)) ||
      ...
      if (isSelectAll) {
        ...
        // --- FUZZY SCANNING LOGIC ---
        let tempQuery = " " + cleanQuery + " ";

        // 1. SCOPED CATEGORY PROTECTION
        // Detect if user said "[Category] mein [Item]" or similar.
        // If they did, we mark the category name as [DONE] so it doesn't trigger "Add All".
        const connectors = [
          "mein",
          "me",
          "in",
        ...
        menus.forEach((cat) => {
          ...
          connectors.forEach((conn) => {
            const scopedPattern = " " + normalizedCatName + " " + conn + " ";
            if (tempQuery.includes(scopedPattern)) {
              // Mark category + connector as [DONE] to protect it from "Add All"
              tempQuery = tempQuery.replace(scopedPattern, " [DONE] ");
            }
          });
        });

        // 2. ITEM SCANNING (Fuzzy & Multi-item)
        const allItems: any[] = [];
        menus.forEach((cat) =>
        ...
        sortedItems.forEach((item) => {
          const normalizedItemName = item.name
            .toLowerCase()
            .replace(/[^\w\s]/gi, "");
          const itemWordCount = normalizedItemName.split(/\s+/).length;

          const currentWords = tempQuery.trim().split(/\s+/);

          for (let i = 0; i <= currentWords.length - itemWordCount; i++) {
            const window = currentWords.slice(i, i + itemWordCount).join(" ");
            if (window.includes("[DONE]")) continue;

            if (
              window === normalizedItemName ||
              getSimilarity(window, normalizedItemName) >= 0.75
            ) {
              let qty = 1;
              const matchPattern = " " + window + " ";
              const matchIdx = tempQuery.indexOf(matchPattern);

              if (matchIdx !== -1) {
                const wordsBefore = tempQuery
                  .substring(0, matchIdx)
                  .trim()
                  .split(/\s+/);
                const lastWordBefore = wordsBefore[wordsBefore.length - 1];
                const wordsAfter = tempQuery
                  .substring(matchIdx + matchPattern.length)
                  .trim()
                  .split(/\s+/);
                const firstWordAfter = wordsAfter[0];

                if (QUANTITY_MAP[lastWordBefore] !== undefined) {
                  qty = QUANTITY_MAP[lastWordBefore];
                } else if (QUANTITY_MAP[firstWordAfter] !== undefined) {
                  qty = QUANTITY_MAP[firstWordAfter];
                }

                matches.push({ item, qty });
                tempQuery = tempQuery.replace(matchPattern, " [DONE] ");
                break;
              }
            }
          }
        ...
        // 3. CATEGORY SCANNING (Add All)
        const sortedCats = [...menus].sort(
          (a, b) => (b.name || "").length - (a.name || "").length,
        );
        sortedCats.forEach((cat) => {
          const normalizedCatName = (cat.name || "")
            .toLowerCase()
            .replace(/[^\w\s]/gi, "");
          const catWordCount = normalizedCatName.split(/\s+/).length;
          const currentWords = tempQuery.trim().split(/\s+/);

          for (let i = 0; i <= currentWords.length - catWordCount; i++) {
            const window = currentWords.slice(i, i + catWordCount).join(" ");
            if (window.includes("[DONE]")) continue;

            if (
              window === normalizedCatName ||
              getSimilarity(window, normalizedCatName) >= 0.75
            ) {
              cat.items?.forEach((item: any) => {
                matches.push({ item, qty: 1, categoryName: cat.name });
              });
              tempQuery = tempQuery.replace(" " + window + " ", " [DONE] ");
              break;
            }
          }
        });
      ...
      if (matches.length > 0) {
        ...
        if (!globalHasAdded) {
          ...
          const totalSnapshot = processedItemsSnapshot.reduce(
            (s, i) => s + (i.price || 0) * i.quantity,
            0,
          );

          setTimeout(() => {
            if (shouldSave && onSaveRequested)
              onSaveRequested(processedItemsSnapshot, totalSnapshot);
            if (shouldKOT || shouldGenericPrint) {
              if (onKOTRequested)
                onKOTRequested(processedItemsSnapshot, totalSnapshot);
            }
            if (shouldBill && onBillRequested)
              onBillRequested(processedItemsSnapshot, totalSnapshot);
          }, 800);
        }
        ...
        if (!globalHasSpoken && now - globalLastSpokenTime > 2000) {
          ...
          try {
            ...
            if (ExpoSpeech && typeof ExpoSpeech.speak === "function") {
              ...
              matches.forEach((m) => {
                if (m.categoryName && m.categoryName !== "All Items") {
                  if (!categoriesSeen.has(m.categoryName)) {
                    groups.push(`all from ${m.categoryName}`);
                    categoriesSeen.add(m.categoryName);
                  }
                } else if (m.categoryName !== "All Items") {
                  groups.push(
                    m.qty > 1 ? `${m.qty} ${m.item.name}` : m.item.name,
                  );
                }
              });

              let confirmText = isSelectAll
                ? `Added all ${matches.length} items.`
                : "Added " + groups.join(", ");

              if (shouldSave) confirmText += " and saved.";
              if (shouldKOT || shouldGenericPrint)
                confirmText += " printing ticket.";
              if (shouldBill) confirmText += " and billing.";
        ...
        let recognizedLabel =
          matches.length > 3
            ? `Added ${matches.length} Items`
            : "Added: " +
              matches.map((m) => `${m.qty}x ${m.item.name}`).join(", ");
        ...
        if (!globalHasSpoken && now - globalLastSpokenTime > 2000) {
          ...
          try {
            const ExpoSpeech = require("expo-speech");
            ExpoSpeech.speak("Sorry, items not found. Please try again.", {
              language: "hi-IN",
              pitch: 1.0,
              rate: 0.9,
            });
            ...
            <View style={styles.micWrapper}>
              <TouchableOpacity
                ...
                style={[
                  ...
                  !voiceLoaded && { backgroundColor: "#94a3b8" },
                ]}
