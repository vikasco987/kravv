...
const MainClientView = ({
  ...
  const [activeTab, setActiveTab] = useState<"parties" | "categories">(
    "parties",
  );
  ...
  const fetchBills = async () => {
    ...
    try {
      ...
      const url = bId
        ? `https://billing.kravy.in/api/bill-manager?t=${Date.now()}&businessId=${bId}`
        : `https://billing.kravy.in/api/bill-manager?t=${Date.now()}`;
  ...
  const customerStats = React.useMemo(() => {
    if (!selectedParty || !allBills || allBills.length === 0)
      return { lifetimeSpend: 0, pending: 0 };
    ...
    relatedBills.forEach((bill: any) => {
      ...
      const netTaxableVal =
        ...
        (serviceChargeEnabled
          ? (totalTaxable - dAmt) * (serviceChargeRate / 100)
          : 0);
      const avgGstRate = totalTaxable > 0 ? totalGst / totalTaxable : 0;
      const calculatedTotal =
        Math.floor((netTaxableVal + netTaxableVal * avgGstRate) * 100) / 100;
  ...
  const partiesStatsMap = React.useMemo(() => {
    ...
    allBills.forEach((bill: any) => {
      ...
      const calcTotal =
        Math.floor(
          (net + net * (billTaxable > 0 ? billGst / billTaxable : 0)) * 100,
        ) / 100;
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            ...
            const stats = partiesStatsMap[pId] ||
              partiesStatsMap[`phone_${cleanP}`] || { lifetimeSpend: 0 };
            return (
              <PartyListItem
                item={item}
