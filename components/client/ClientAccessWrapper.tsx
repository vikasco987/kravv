...
const ClientAccessWrapper = () => {
  ...
  useEffect(() => {
    const checkAccess = async () => {
      // Checking for broad "Client" or "Customer" access
      const allowed = await StaffPermissionEngine.hasPermission("Customer", isOwner);
  ...
  if (!isOwner && !session) {
    return <MainClientView isLockedUser={true} />;
  }

  // CASE 2: Staff Logged In but No Client Permission
  if (!hasAccess) {
    return <NoAccessView />;
  }

  // CASE 3: Owner or Staff with Permission
  return <MainClientView isLockedUser={false} />;
};
