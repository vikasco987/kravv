...
const MonthlyItemSalesReport = ({ onBack, preloadedData, loadingData, onRefresh: parentRefresh, targetSortKey }: any) => {
  ...
  const { monthlyBills, reportData } = React.useMemo(() => {
    if (!preloadedData) return { monthlyBills: [], reportData: { items: [], totalSales: 0, totalQty: 0 } };
    ...
    return { monthlyBills: monthlyBillsList, reportData: { items: sortedItems, totalSales: totalRevenue, totalQty: totalItemsSold } };
  }, [preloadedData]);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#f9fafb' }}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{targetSortKey ? `Item Sales (${new Date(targetSortKey + '-01').toLocaleDateString('en-IN', { month: 'long', year: 'numeric' })})` : "Monthly Item Sales"}</Text>
      </View>
          ...
          {(() => {
            ...
            return filtered.length > 0 ? (
              ...
              <View style={styles.emptyContainer}>
                ...
                <Text style={styles.emptyText}>{selectedCategory === "All" ? "No sales recorded for this period." : `No sales in ${selectedCategory} category.`}</Text>
              </View>
            );
          })()}
        </ScrollView>
