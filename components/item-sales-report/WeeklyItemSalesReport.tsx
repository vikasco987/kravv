...
const WeeklyItemSalesReport = ({ onBack, preloadedData, loadingData, onRefresh: parentRefresh, targetSortKey }: any) => {
  ...
  const { weeklyBills, reportData } = React.useMemo(() => {
    ...
    return { weeklyBills: weeklyBillsList, reportData: { items: sortedItems, totalSales: totalRevenue, totalQty: totalItemsSold } };
  }, [preloadedData]);
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#f9fafb' }}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{targetSortKey ? `Item Sales (${targetSortKey.replace('-W', ' Week ')})` : "Weekly Item Sales"}</Text>
      </View>
