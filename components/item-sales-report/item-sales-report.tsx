...
const ItemSalesReport = ({
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      const catUrl = bId
        ? `https://billing.kravy.in/api/categories?t=${Date.now()}&businessId=${bId}`
        : `https://billing.kravy.in/api/categories?t=${Date.now()}`;
      const menuUrl = bId
        ? `https://billing.kravy.in/api/menu/view?t=${Date.now()}&businessId=${bId}`
        : `https://billing.kravy.in/api/menu/view?t=${Date.now()}`;
      const billUrl = bId
        ? `https://billing.kravy.in/api/bill-manager?t=${Date.now()}&businessId=${bId}&limit=2000`
        : `https://billing.kravy.in/api/bill-manager?t=${Date.now()}&limit=2000`;
      ...
      const fetchedData = {
        categories: cats,
        itemCategoryMap: mapping,
        allBills: bills,
      };
      setSharedData(fetchedData);
      AsyncStorage.setItem(
        "cached_sales_report_data",
  ...
  if (activeReport === "daily") {
    ...
    activeView = (
      ...
      >
        ...
        <TouchableOpacity
          onPress={onBack}
          style={{ marginTop: vs(20), padding: s(10) }}
        >
