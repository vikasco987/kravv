...
const StaffTransferModal = ({
  ...
  const handleUpdate = () => {
    if (staff && name.trim() && email.trim()) {
      onUpdate(staff.id, name, email);
    }
  };
          ...
          <View style={{ gap: vs(15) }}>
            ...
            <View style={subStyles.transferInputBox}>
              <TextInput
                ...
                style={{ fontSize: rf(16), color: COLORS.text }}
              />
            </View>
            <View style={subStyles.transferInputBox}>
              <TextInput
                placeholder="Transfer Staff Email"
                placeholderTextColor="#9CA3AF"
                value={email}
...
export const StaffModal = ({ visible, onClose }: StaffModalProps) => {
  ...
  const [transferringStaff, setTransferringStaff] =
  ...
  const loadStaffData = async () => {
    ...
    try {
      ...
      if (token && bId) {
        const res = await staffService.getStaffList(bId, token);
        if (res.success) {
  ...
  const handleTransferPress = (staff: StaffMember) => {
  ...
  const handleUpdateStaffTransfer = async (
    id: string,
    name: string,
    email: string,
  ) => {
    ...
    <Modal visible={visible} animationType="slide" onRequestClose={onClose}>
      <SafeAreaView style={styles.safeArea}>
          ...
          {searchMode ? (
            ...
            <>
              ...
              <View style={{ flex: 1, marginLeft: s(10) }}>
                ...
                <Text
                  style={{ fontSize: rf(11), color: "rgba(255,255,255,0.7)" }}
                >
                  FAST v39.08 | 7838491780 | 3307
                </Text>
              </View>
              <TouchableOpacity
        ...
        >
          <View style={styles.settingRow}>
            <View style={{ flex: 1 }}>
              ...
              >
                {assignEnabled ? "Enabled" : "Disabled"}
              </Text>
              <Text style={{ fontSize: rf(12), color: COLORS.success }}>
                This will enable admin users to assign staff to the invoice for
                sales tracking
              </Text>
            </View>
            <Switch
              value={assignEnabled}
              onValueChange={toggleAssign}
          ...
          <View style={styles.statsCard}>
            <Text style={styles.statsLabel}>
              Staff Can be Added:-{" "}
              <Text style={{ color: COLORS.success, fontWeight: "bold" }}>
                {Math.max(0, 10 - staffList.length)}
              </Text>
            </Text>
          </View>
          {loading ? (
                ...
                >
                  ...
                  >
                    <View style={[styles.badge, { flex: 1 }]}>
                      <Text style={{ fontSize: rf(14) }}>{item.name}</Text>
                    </View>
                    <View style={[styles.badge, { flex: 1 }]}>
                      <Text style={{ fontSize: rf(14) }} numberOfLines={1}>
                        {item.email}
                      </Text>
                    </View>
                    <TouchableOpacity
                  ...
                  <View style={styles.permRow}>
                    ...
                    >
                      {item.permissions.length} OUT OF {TOTAL_PERMISSIONS_COUNT}{" "}
                      PERMISSIONS GRANTED
                    </Text>
                    <Ionicons
                      name="checkmark-done"
        ...
        <StaffTransferModal
          ...
          onUpdate={handleUpdateStaffTransfer}
