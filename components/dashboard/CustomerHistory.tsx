...
const CustomerHistory = ({
    ...
    const subscriptions = [
      ...
      DeviceEventEmitter.addListener("onSpeechResults", (e: any) => {
        if (e.value && e.value.length > 0) {
          ...
          const now = Date.now();
          if (
            !customerSearchHasSpoken &&
            now - customerSearchLastSpokenTime > 2000
          ) {
            ...
            try {
              ...
              if (ExpoSpeech && typeof ExpoSpeech.speak === "function") {
                ...
                const hasMatch = (bills || []).some((bill) => {
                  const bName = (bill.customerName || bill.name || "")
                    .toLowerCase()
                    .trim();
                  const bPhoneRaw = (
                    bill.customerPhone ||
                  ...
                  const cleanBillPhone =
                    bPhoneRaw.length > 10 ? bPhoneRaw.slice(-10) : bPhoneRaw;
                  ...
                  if (queryNumeric && queryNumeric.length >= 7) {
                    const cleanQueryPhone =
                      queryNumeric.length > 10
                        ? queryNumeric.slice(-10)
                        : queryNumeric;
                    if (
                      cleanBillPhone &&
                      cleanBillPhone.includes(cleanQueryPhone)
                    )
                      return true;
                  }
  ...
  const insights = useMemo(() => {
    ...
    const cleanTargetPhone =
      targetPhone.length > 10 ? targetPhone.slice(-10) : targetPhone;
    ...
    const customerBills = (bills || []).filter((bill) => {
      ...
      const bPhoneRaw = (bill.customerPhone || bill.phone || "").replace(
        /\D/g,
        "",
      );
      const cleanBillPhone =
      ...
      if (party) {
        ...
        if (
          cleanTargetPhone &&
          cleanBillPhone &&
          cleanTargetPhone === cleanBillPhone
        )
          return true;
        ...
        if (
          targetName &&
          bName &&
          (bName.includes(targetName) || targetName.includes(bName))
        )
          return true;
        return false;
      }
      ...
      if (query) {
        ...
        const cleanSuffixQuery =
          cleanQuery.length > 10 ? cleanQuery.slice(-10) : cleanQuery;

        // If it's a numeric search (Phone)
        if (cleanQuery) {
          // Normalize both sides to find the best match (handles +91 vs no +91)
          if (cleanBillPhone && cleanBillPhone.includes(cleanSuffixQuery))
            return true;
        }
    ...
    customerBills.forEach((bill) => {
      ...
      const calculatedTotal =
        Math.floor((netTaxableVal + finalGstAmt) * 100) / 100;
      const storedTotal = Number(
        bill.total || bill.grandTotal || bill.totalAmount || 0,
      );
    ...
    const sortedItems = Object.values(itemMap)
      ...
      .slice(0, 5);
    const sortedByDate = [...customerBills].sort(
      (a, b) =>
    ...
    const displayName =
      party?.name || latestBill?.customerName || latestBill?.name || "Customer";
  ...
  return (
    ...
    >
        ...
        <View style={styles.modalContainer}>
          <View style={styles.header}>
            <View>
              ...
              <Text style={styles.subTitle}>
                {bills?.length || 0} Records Analyzed
              </Text>
            </View>
                ...
                <View style={styles.statsRow}>
                  ...
                  <View style={[styles.statBox, styles.statDivider]}>
                    ...
                    <Text style={[styles.statValue, { color: "#10b981" }]}>
                      ₹{(insights.totalSpend || 0).toFixed(2)}
                    </Text>
                  </View>
                  <View style={styles.statBox}>
                    ...
                    <Text style={styles.statValue}>
                      {insights.lastVisit
                        ? new Date(insights.lastVisit).toLocaleDateString()
                        : "N/A"}
                    </Text>
                  </View>
                </View>
                ...
                <View style={styles.section}>
                  <View style={styles.sectionHeader}>
                    ...
                    <Text style={styles.sectionTitle}>
                      {insights.favoriteItems.length === 1
                        ? "Top Favorite Dish:"
                        : "Customer Favorites:"}
                    </Text>
                  </View>
                    ...
                    {insights.favoriteItems.map((it: any, idx: number) => {
                      ...
                      return (
                        <TouchableOpacity
                          ...
                          style={[
                            styles.favRow,
                            isSelected && styles.favRowSelected,
                          ]}
                        ...
                        >
                          ...
                          <Text
                            style={[
                              styles.favName,
                              isSelected && styles.favNameSelected,
                            ]}
                            numberOfLines={1}
                          >
                            {it.name}
          ...
          {selectedItemNames.length > 0 ? (
            <TouchableOpacity
              ...
              onPress={() => handleBatchAction(selectedItemNames)}
            >
              <Text style={styles.confirmBtnText}>
                Select & Add {selectedItemNames.length} Items
              </Text>
            </TouchableOpacity>
          ) : (
      ...
      <Modal visible={voiceModalVisible} transparent animationType="slide">
        <View
          style={[styles.overlay, { justifyContent: "flex-end", padding: 0 }]}
        >
          <View
            style={[
