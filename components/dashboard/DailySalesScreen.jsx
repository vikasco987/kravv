...
const SalesCard = ({ date, numberOfBills, totalSales, t }) => (
  <View style={enhancedStyles.card}>
    ...
    <View style={enhancedStyles.cardBody}>
      ...
      />
      <SalesStat
        label={t("total_sales_label") || "Total Sales"}
  ...
  <View style={enhancedStyles.tableContainer}>
    ...
    <FlatList
      ...
      renderItem={({ item }) => (
        ...
        >
          <Text style={[enhancedStyles.tableCell, { flex: 3 }]}>
            {item.date}
          </Text>
      ...
      ListEmptyComponent={() => (
        <View style={enhancedStyles.emptyContainer}>
          <Text style={enhancedStyles.emptyText}>
            {t("no_sales_found_table") || "No sales found for Table View."}
          </Text>
        </View>
      )}
    />
  ...
  const fetchBills = async (silent = false) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setDailySales(
          groupSalesByDate(billsList.filter((b) => b.isHeld !== true)),
        );
      }
    } catch (err) {
      const errorMsg =
      ...
      <View style={{ flex: 1, paddingTop: vs(10) }}>
        {viewMode === "card" ? (
          <FlatList
            ...
            renderItem={({ item }) => (
              <TouchableOpacity
                onPress={() => setSelectedDateReport(item.id)}
                activeOpacity={0.8}
              >
                <SalesCard
                  date={item.date}
