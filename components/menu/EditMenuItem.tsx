...
export const EditMenuItem = ({ onBack }: { onBack: () => void }) => {
  ...
  const [showAddItemCategoryScreen, setShowAddItemCategoryScreen] =
    useState(false);
  ...
  const [showDeleteCategoryConfirm, setShowDeleteCategoryConfirm] =
    useState(false);
  const [showUpdateCategorySuccess, setShowUpdateCategorySuccess] =
    useState(false);
  ...
  const fetchMenus = useCallback(
    async (isManual = false) => {
      ...
      try {
        ...
        const finalMenus = Object.values(categoryMap).sort((a, b) =>
  ...
  useEffect(() => {
    if (isLoaded) {
      ...
      Promise.all([fetchMenus(), fetchAllCategories()]);
    }
  ...
  const handleItemClick = async (
    ...
    try {
      ...
      const url = bId
        ? `https://billing.kravy.in/api/items?id=${item.id}&businessId=${bId}`
        : `https://billing.kravy.in/api/items?id=${item.id}`;
    ...
    setMenus((prev) =>
      prev
        ...
        .filter(
          (cat) =>
            cat.items.length > 0 ||
            allCategories.some((ac) => ac.id === cat.id),
        ),
    );
  ...
  const handleUpdateCategory = async () => {
    ...
    try {
      ...
      setMenus((prev) =>
        prev.map((cat) =>
          cat.id === selectedCategoryToEdit.id
            ? { ...cat, name: editCategoryNewName }
            : cat,
        ),
      );
  ...
  const handleDeleteCategory = async () => {
    ...
    try {
      ...
      setMenus((prev) =>
        prev.filter((cat) => cat.id !== selectedCategoryToEdit.id),
      );
      setAllCategories((prev) =>
        prev.filter((cat) => cat.id !== selectedCategoryToEdit.id),
      );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (
        editImageUrl.startsWith("file://") ||
        editImageUrl.startsWith("content://")
      ) {
        finalImageUrl = await uploadToCloudinary(editImageUrl).catch(
          () => editImageUrl,
        );
      }
  ...
  const filteredMenus = React.useMemo(() => {
    ...
    let result = displayCategories
      .map((catBase) => {
        ...
        const filteredItems = itemsSource.filter((i) =>
        ...
        if (
          searchQuery &&
          !(
            catBase.name.toLowerCase().includes(query) ||
        ...
        return {
          ...
          name: catBase.name,
          items:
            searchQuery && !catBase.name.toLowerCase().includes(query)
              ? filteredItems
              : itemsSource,
        };
      })
  ...
  return (
    <SafeAreaView style={styles.container}>
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptyText}>
                {searchQuery
                  ? t("no_search_results") || "No items match your search"
                  : t("no_items_category") || "No items in this category"}
              </Text>
            </View>
          }
        />
            ...
            <View style={{ paddingVertical: vs(10) }}>
              ...
              <TouchableOpacity
                style={[
                  styles.saveBtn,
                  { marginTop: vs(24) },
                  isCreatingCategory && { opacity: 0.7 },
                ]}
                onPress={async () => {
                  ...
                  try {
                    ...
                    const authToken = await getToken();
                    const sessionStr =
                      await AsyncStorage.getItem("staff_session");
                    const staffSession = sessionStr
                    ...
                    Alert.alert(
                      "Error",
                      "Failed to create category (Server Timeout). Please try again or refresh.",
                    );
                  } finally {
      ...
      <Modal
        ...
        animationType="fade"
        onRequestClose={() => setShowCategorySuccess(false)}
      >
        <View style={styles.modalOverlayCentered}>
      ...
      <Modal
        ...
        animationType="fade"
        onRequestClose={() => setShowUpdateSuccess(false)}
      >
        <View style={styles.modalOverlayCentered}>
          <View style={styles.feedbackModalContent}>
            <View style={{ alignItems: "center" }}>
        ...
        <View style={styles.modalOverlayCentered}>
          <View style={styles.confirmModalContent}>
            <View style={{ alignItems: "center" }}>
              ...
              <Text style={[styles.feedbackDetail, { marginVertical: vs(10) }]}>
              ...
              >
                ...
                <TouchableOpacity
                  style={[
                    styles.modalActionBtn,
                    { backgroundColor: "#EF4444", marginLeft: s(10) },
                  ]}
          ...
          <View style={styles.feedbackModalContent}>
            <View style={{ alignItems: "center" }}>
              <View
                style={[
                  styles.successCircle,
                  { backgroundColor: "#FFEDD5", borderColor: "#FED7AA" },
                ]}
              >
                <Ionicons name="warning" size={rf(36)} color="#F97316" />
              </View>
              <Text style={[styles.feedbackTitle, { color: "#F97316" }]}>
                {validationMsg.title}
              </Text>
              ...
              <View style={styles.modalContent}>
                <View style={styles.modalHeaderNoMargin}>
                  <Text style={styles.modalTitle}>
                    {t("select_categories") || "Choose Category"}
                  </Text>
                ...
                <FlatList
                  ...
                  ListEmptyComponent={
                    <View style={{ padding: s(20), alignItems: "center" }}>
                      <Text style={{ color: COLORS.GRAY, fontSize: rf(14) }}>
                        {t("no_items_category") || "No categories found."}
                      </Text>
                    </View>
                  }
                />
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.WHITE }}>
              ...
              {selectedCategoryToEdit?.items.map((item, index) => (
                <View
                  ...
                  style={{
                    ...
                    borderBottomWidth:
                      index === selectedCategoryToEdit.items.length - 1 ? 0 : 1,
                    borderBottomColor: "#F3F4F6",
                  }}
                >
                  <Text style={{ fontSize: rf(16), color: COLORS.SECONDARY }}>
                    {item.name}
                  </Text>
                </View>
              ))}
          ...
          <View style={styles.modalFixedFooter}>
            ...
            <TouchableOpacity
              style={[
                styles.modalActionBtn,
                { backgroundColor: COLORS.PRIMARY, marginLeft: s(15) },
              ]}
              onPress={() => {
                ...
                <View style={{ alignItems: "center" }}>
                  ...
                  <Text
                    style={[
                      ...
                      { marginVertical: vs(10), lineHeight: rf(20) },
                    ]}
                  >
                    Are you sure you want to delete the item category? This
                    action is irreversible. All items within this category will
                    be moved to Uncategorised
                  </Text>
          ...
          <Modal
            transparent={true}
            visible={showUpdateCategorySuccess}
            animationType="fade"
          >
            <View style={styles.modalOverlayCentered}>
              <View style={styles.feedbackModalContent}>
          ...
          <Modal
            transparent={true}
            visible={showDeleteCategorySuccess}
            animationType="fade"
          >
            <View style={styles.modalOverlayCentered}>
              <View
              ...
              >
                <View
                  style={[styles.successCircle, { backgroundColor: "#FEE2E2" }]}
                >
                  <Ionicons name="trash" size={rf(36)} color="#EF4444" />
                </View>
                <Text style={[styles.feedbackTitle, { color: "#EF4444" }]}>
